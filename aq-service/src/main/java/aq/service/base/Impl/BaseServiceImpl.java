...
public class BaseServiceImpl implements BaseService {
    ...
    @Override
    public JsonObject queryNoPage(JsonObject jsonObject, Function<Map<String, Object>, List<Map<String, Object>>> func) {
        Rtn rtn = new Rtn(jsonObject.get("service").getAsString());
        JsonObject data = new JsonObject();
        JsonArray jsonArray = new JsonArray();
        data.add("items",jsonArray);
        ...
        List<Map<String, Object>> apply = func.apply(map);
        rtn.setCode(200);
        rtn.setMessage("success");
        jsonArray =  GsonHelper.getInstanceJsonparser().parse(GsonHelper.getInstance().toJson(apply)).getAsJsonArray();
        data.add("items",jsonArray);
        rtn.setFrom(jsonObject.get("from")==null?"":jsonObject.get("from").getAsString());
        rtn.setData(data);
        return  Func.functionRtnToJsonObject.apply(rtn);
